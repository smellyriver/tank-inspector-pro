    ...
    */
    ...
    public class Catalog
    {
        ...
        protected Catalog(List<string> originalStrings, List<string> translationStrings)
        {
            ...
            {
                StringTable[originalStrings[i]] = translationStrings[i];
            }
        }
        ...
        {
            string text;
            if (StringTable.TryGetValue(messageId, out text))
            {
                return text;
            }
            else
            {
                return string.Format("$missing: {0}", messageId);
            }
        }
        ...
        public static Catalog ReadFrom(Stream stream)
        {
            using (var br = new FastBinaryReader(stream))
            {
                uint magicNum = br.ReadUint32();
                uint version = br.ReadUint32();
                Debug.Assert(magicNum == 0x950412de);
                ...
                return new Catalog(originalStrings, translationStrings);
            }
        }

        public static Catalog ReadFrom(string path)
        {
            using (var file = File.OpenRead(path))
                return Catalog.ReadFrom(file);
        }
    }
}
