...
namespace Smellyriver.TankInspector.Pro.GameClientExplorer.FileSystem
{
    ...
    public class ShellContextMenu : NativeWindow
    {
        ...
        public static void ShowFolderContextMenu(string path)
        {
            new ShellContextMenu().ShowContextMenu(new[] { new DirectoryInfo(path) }, Control.MousePosition);
        }

        public static void ShowFolderContextMenu(string path, int x, int y)
        {
            Point position = new Point(x, y);
            new ShellContextMenu().ShowContextMenu(new[] { new DirectoryInfo(path) }, position);
        }

        public static void ShowFileContextMenu(string path)
        {
            new ShellContextMenu().ShowContextMenu(new[] { new FileInfo(path) }, Control.MousePosition);
        }

        public static void ShowFileContextMenu(string path, int x, int y)
        {
            Point position = new Point(x, y);
            new ShellContextMenu().ShowContextMenu(new[] { new FileInfo(path) }, position);
        }
        ...
        private IShellFolder GetDesktopFolder()
        {
            ...
            if (null == _oDesktopFolder)
            {
                ...
                int nResult = ShellContextMenu.SHGetDesktopFolder(out pUnkownDesktopFolder);
                if (S_OK != nResult)
                {
        ...
        private IShellFolder GetParentFolder(string folderName)
        {
            if (null == _oParentFolder)
            {
                ...
                ShellContextMenu.StrRetToBuf(pStrRet, pPIDL, strFolder, MAX_PATH);
        ...
        private void InvokeContextMenuDefault(FileInfo[] arrFI)
        {
            ...
            try
            {
                ...
                pMenu = ShellContextMenu.CreatePopupMenu();
        ...
        private void ShowContextMenu(Point pointScreen)
        {
            ...
            try
            {
                ...
                uint nSelected = ShellContextMenu.TrackPopupMenuEx(
                    pMenu,
                    TPM.RETURNCMD,
                    pointScreen.X,
                    pointScreen.Y,
    ...
    public class LocalWindowsHook
    {
        ...
        {
            if (code < 0)
                return LocalWindowsHook.CallNextHookEx(m_hhook, code, wParam, lParam);
        ...
        public void Install()
        {
            m_hhook = LocalWindowsHook.SetWindowsHookEx(
                m_hookType,
                m_filterFunc,
                IntPtr.Zero,
        ...
        {
            LocalWindowsHook.UnhookWindowsHookEx(m_hhook);
        }
        // ************************************************************************
