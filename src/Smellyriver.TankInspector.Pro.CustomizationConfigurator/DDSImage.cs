...
namespace Smellyriver.TankInspector.Pro.Graphics
{
    class DDSBitmap
    {
        ...
        {
            data[x + y * Width] = finalColor;
        }

        public BitmapSource ToBitmapSource()
        {
            var bitmapSource = BitmapSource.Create(Width, Height, 0, 0, PixelFormats.Pbgra32, null, data, Width * sizeof(int));
    ...
    struct Color
    {
        ...
        private const byte c_shortToIntMultipler = 0xff / 0xf;
        ...
        public unsafe static int FromShort(ushort color)
        {
            byte* bytes = stackalloc byte[4];

            bytes[0] = (byte)((color & 0xF) * c_shortToIntMultipler);
            bytes[1] = (byte)(((color & 0xF0) >> 4) * c_shortToIntMultipler);
            bytes[2] = (byte)(((color & 0xF00) >> 8) * c_shortToIntMultipler);
            bytes[3] = (byte)(((color & 0xF000) >> 12) * c_shortToIntMultipler);
    ...
    public class DDSImage
    {
        ...
        private const int FOURCC_DXT4 = 0x33545844;
        ...
        public BitmapSource BitmapSource(int level)
        {
            ...
            if (images.Length > level)
                return images[level].ToBitmapSource();
            return null;
        ...
        public DDSImage(Stream stream)
        {
            using (BinaryReader r = new BinaryReader(stream))
            {
                ...
                images = new DDSBitmap[mipMapCount];
            ...
            fixed (byte* pData = data)
            {
                switch (header.ddspf.dwFourCC)
                {
                    case FOURCC_DXT1:
                        ...
                        return UncompressDXT1(pData, w, h);

                    case FOURCC_DXT5:
                        return UncompressDXT5(pData, w, h);
                    case FOURCC_DXT4:
                        return UncompressDXT5(pData, w, h);
                    default: break;
                }
        ...
        private unsafe DDSBitmap UncompressDXT5(byte* data, int w, int h)
        {
            ...
            for (int j = 0; j < blockCountY; j++)
            {
                ...
                for (int i = 0; i < blockCountX; i++)
                {
                    ...
                    DecompressBlockDXT5(i * 4, j * 4, w, h, data, res);
                    data += 16;
                }
            }
