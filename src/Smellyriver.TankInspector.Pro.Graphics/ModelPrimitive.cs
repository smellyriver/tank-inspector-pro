...
namespace Smellyriver.TankInspector.Pro.Graphics
{
    public static class VertexFormatExtension
    {
        ...
        {
            return vf == ModelPrimitive.VertexFormat.xyznuviiiwwtb || vf == ModelPrimitive.VertexFormat.set3_xyznuviiiwwtbpc;
        }
    }
    ...
    public class ModelPrimitive
    {
        ...
        public enum VertexFormat
        {
            ...
            xyznuviiiwwtb,
            xyznuv,
            set3_xyznuviiiwwtbpc,
            set3_xyznuvtbpc,
            set3_xyznuvpc,
        }
            
        public Dictionary<string, VerticesList> Vertices { get; private set; }

        public Dictionary<string, IList<int>> Indices { get; private set; }
        ...
        public static ModelPrimitive ReadFrom(Stream stream, ModelVisual visual, bool recalculateNormals)
        {
            ...
            foreach (var section in sections)
            {
                ...
                {
                    var vertices = ReadVerticesFrom(stream, section, recalculateNormals);
                ...
                else
                {
                    ...
                    if (renderSets.Count != 0)
                    {
                        var indices = ReadIndicesFrom(stream, section, renderSets);
            ...
            if (recalculateNormals)
            {
                foreach (var renderSet in visual.RenderSets)
                {
                    ...
                    for (int i = 0; i != indices.Count; i += 3)
                    {
                        ...
                        var normal = CalculateNormal(vertices[index0].Position, vertices[index1].Position, vertices[index2].Position);
        ...
        private static IList<int> ReadIndicesFrom(Stream stream, PrimitiveSection section,IList<ModelRenderSet> renderSets)
        {
            ...
            {
                throw new InvalidDataException(string.Format("this index type is {0},only support list and list32 index type ", indexSizeType));
            }
            ...
            {
                var startIndex_ = reader.ReadUInt32();
                var nPrimitives_ = reader.ReadUInt32();
                var startVertex_ = reader.ReadUInt32();
                var nVertices_ = reader.ReadUInt32();
                foreach (var renderSet in renderSets)
                {
                    ModelPrimitiveGroup group;
                    if (renderSet.Geometry.ModelPrimitiveGroups.TryGetValue(i, out group))
                    {
                        group.StartIndex = startIndex_;
                        group.PrimitiveCount = nPrimitives_;
                        group.EndIndex = startIndex_ + nPrimitives_;
                        group.StartVertex = startVertex_;
                        group.VerticesCount = nVertices_;
                        group.EndVertex = startVertex_ + nVertices_;
        ...
        private static unsafe VerticesList ReadVerticesFrom(Stream stream, PrimitiveSection section, bool recalculateNormals)
        {
            ...
            {
                vf = VertexFormat.set3_xyznuviiiwwtbpc;
                //SIZE = 40
            ...
            else if(vertexFormat == "set3/xyznuvtbpc" || vertexFormat == "setxyznuvtbpc")
            {
                ...
                vf = VertexFormat.set3_xyznuvtbpc;
            }
            else if(vertexFormat == "set3/xyznuvpc" || vertexFormat == "setxyznuvpc")
            {
                //    SIZE = 24
                //UNPACK_FORMAT = '<3fI2f'
                vf = VertexFormat.set3_xyznuvpc;
            }
            ...
            for (int i = 0; i != vertexCount; ++i)
            {
                ...
                if (!recalculateNormals)
                {
                    v.Normal = newVersion ? UnpackNormal_tag3(normal) : UnpackNormal(normal);
                }
                ...
                if (vf.IsSkinned())
                {
                    ...
                    if (vf == VertexFormat.set3_xyznuviiiwwtbpc)
                    {
                ...
                if (vf != VertexFormat.xyznuv && vf != VertexFormat.set3_xyznuvpc)
                {
                    ...
                    if (!recalculateNormals)
                    {
                        ...
                        v.Tangent = newVersion ? UnpackNormal_tag3(tangent) : UnpackNormal(tangent);
                        v.Binormal = newVersion ? UnpackNormal_tag3(binormal) : UnpackNormal(binormal);
                        if (vf.IsSkinned())
                        {
