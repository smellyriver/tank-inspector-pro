...
sampler2D camouflageExclusionMaskSampler : register(S4)=
	sampler_state
{
    MAGFILTER = ANISOTROPIC;
    MINFILTER = ANISOTROPIC;
...
sampler2D camouflageSampler : register(S5)=
	sampler_state
{
    MAGFILTER = ANISOTROPIC;
    MINFILTER = ANISOTROPIC;
...
PS_OUT PixelShaderFunction(PS_IN input)
{
	...
	if(useDiffuse)
	{
		...
		if(showCamouflage)
		{
			float4 camouflageExclusionMask = tex2D(camouflageExclusionMaskSampler,input.uv);

			float4 camouflage = tex2D(camouflageSampler,input.uv); 

			color = camouflageExclusionMask.rgb  + diffuseColor.rgb;
		}
		else
		{
